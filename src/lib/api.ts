...
const API_URL =
  ...
  'https://admin.lambtonphones.com.au'
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 500) {
        console.warn(
          'CMS Backend is experiencing internal server errors. Please check backend deployment and environment variables.',
        )
      }
...
export async function getBlogs(
  ...
  try {
    const { status = 'published', sort = '-publishedDate', depth = 1 } = options
...
export function validateBlogData(blog: any): blog is BlogPost {
  ...
  return requiredFields.every(
    (field) => blog.hasOwnProperty(field) && blog[field] !== null && blog[field] !== undefined,
  )
}
